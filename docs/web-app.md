...
- Printers grouped by model: MK4S → Core One → Core 1L → XL → Other (in that order). Each model gets its own self-contained chip rather than a full-width row per model: a fleet with many single- or two-printer models packs several narrow chips per line instead of showing mostly-empty full-width rows, matching the Dashboard's `FleetStatusGrid`. A chip's width is a CSS `max(220px, min(100%, N * 230px - 10px, CHIP_MAX_VW vw))`, not a fixed pixel value: exactly as wide as that model's card count needs, capped at `CHIP_MAX_VW` (42) percent of the viewport so a large model wraps internally onto more rows within its own chip instead of growing arbitrarily wide, and never wider than the container so it still shrinks correctly on a narrow viewport. Because the cap is a live `vw` expression rather than a number computed once in JS, it scales with the window automatically (more columns fit per chip on a wide monitor, fewer on a laptop) with no resize listener needed; the `max(220px, ...)` floor exists because `vw` alone can resolve below one card's minimum width on a narrow/mobile viewport.
