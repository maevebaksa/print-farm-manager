...
**Matching (`fuzzyScore` in the component):** an exact substring match always scores best and sorts first, exactly as a plain substring filter always did. Failing that, a small sliding-window edit-distance (Levenshtein) check looks for a close typo'd match anywhere in the name (a missing, extra, wrong, or transposed character, so `"swichwire"` still finds `Switchwire-0`), ranking results by how close the match is. Disabled for queries of 3 characters or fewer, where almost anything would look "close" and the noise isn't worth it; those still need an exact substring. No new dependency: a small dependency-free implementation, not a fuzzy-search library.
...
| Section | Description |
|---|---|
| Header | Branding, fleet utilization % (printing / total), live HH:MM:SS clock and date. `flexWrap`s onto extra lines rather than a fixed height, since a TV/full monitor is the primary target but the browser can still be narrower (e.g. snapped to half a screen); a fixed height previously let overflowing content spill outside the box and overlap the section next to it instead of the row growing or wrapping. |
...
Each card is a `WebcamCard` component (so its rotation-fit natural-size state, see below, is a proper per-image hook instance rather than one object shared across a `.map()`) and shows what's loaded above the image, each line with a colored swatch (`ColorSwatch.jsx`): every `printer_lanes` row (`GET /api/printers`'s `lanes` field) if the printer has klipper-filament-sync, one line per lane, or the legacy `loaded_material`/`loaded_color` otherwise. Both cases source their swatch color the same way: a lane's `color` and the legacy `loaded_color` are both Filament Library color names, looked up through the same `colorHexMap` (`filamentColorHex.js`) as Fleet and Printer Detail.

`streamUrl`/`snapshotUrl`, same as everywhere else a camera feed is shown, are this server's own proxy routes, not the printer's LAN address: see docs/api.md's `GET /:id/camera` entry. A card whose camera reports a `streamUrl` gets a **Watch Live** button. Live view is **single-instance**: the page keeps one `liveViewId` (the printer currently live, or `null`), so clicking Watch Live on a card switches live view to it and implicitly stops whatever other card was live, rather than opening a second concurrent MJPEG stream. Every rendered image (snapshot or live) has `client/src/cameraTransform.js`'s rotation/flip transform applied, the same as the printer detail camera card and the hover preview.
...
- Printers grouped by model: MK4S → Core One → Core 1L → XL → Other (in that order). Each model gets its own self-contained chip, a bordered rectangle (`background: #111827`, `1px solid #1e2433`, matching the Dashboard's `FleetStatusGrid` chips) rather than a full-width row per model: a fleet with many single- or two-printer models packs several narrow chips per line instead of showing mostly-empty full-width rows. A chip's width is a CSS `max(220px + padding, min(100%, N * 230px - 10px + padding, max(CHIP_MAX_VW vw, TWO_UP_MIN_PX)))`, not a fixed pixel value: exactly as wide as that model's card count needs, capped at `CHIP_MAX_VW` (42) percent of the viewport so a large model wraps internally onto more rows within its own chip instead of growing arbitrarily wide, and never wider than the container so it still shrinks correctly on a narrow viewport. The `padding` term (`CHIP_PADDING_X`, 28px) accounts for the chip's own left/right padding, without which a single-card chip's `minmax(220px, 1fr)` grid column would overflow the chip's border. The `max(CHIP_MAX_VW vw, TWO_UP_MIN_PX)` floor exists because the vw cap alone, meant to stop a *large* model's chip from dominating the row, could also resolve below two cards' width on a narrower window (e.g. the browser snapped to half a smaller laptop screen), squeezing a 2+ printer model to a single column even though the window had room for two; `TWO_UP_MIN_PX` is exactly `2 * 230px - 10px + CHIP_PADDING_X`, so a 2-up (or larger) chip never loses its second column to the vw term specifically, only to the `100%`/naturalWidth terms (a genuinely narrow/mobile viewport, or a 1-card group). Because the cap is a live `vw` expression rather than a number computed once in JS, it scales with the window automatically (more columns fit per chip on a wide monitor, fewer on a laptop) with no resize listener needed.
...
- IP address is not shown on cards. The card header (top row) is just the pin button, name, and status badge, kept clear so the name has room; a `klipper` or `octoprint` printer's web-interface link (**Mainsail ↗**, **OctoPrint ↗**, or **OctoEverywhere ↗** if the printer has an `octoeverywhere_url` set: see `client/src/webUiLink.js`) lives in the model/group row below instead, opening in a new tab (`e.stopPropagation()` so it doesn't also trigger the card's own click-to-open-detail behavior)
...
- **Details panel** (per part, toggle with "Details" button): four sections:
  ...
  - *G-code Files*: lists each uploaded file with a thumbnail (`GcodeThumbnail.jsx`, blank if the file has none embedded), filename, printer model badge, and × delete button (with confirm) → `DELETE /api/gcodes/:id`. A G-code uploaded by an account with `requires_print_approval` set also shows a "Pending approval" badge and, for an `admin`/`operator` viewer, an **Approve** button (`POST /api/gcodes/:id/approve`): it stays excluded from dispatch until then. See [docs/auth.md](auth.md)'s Print approval section.
...
**G-code upload wizard** (`client/src/components/GcodeUploadWizard.jsx`): an alternative path into the same upload, for when the destination Part doesn't exist yet or isn't already open in the detail view. Triggered by dropping a file anywhere on either the list view or an open project's detail view (both listen for `dragenter`/`dragover`/`dragleave`/`drop`, ignoring the page's own internal project/part row-reorder drag by checking `dataTransfer.types` for `Files`) or the "+ Upload G-code" header button; all three open the same 3-step modal (destination, then G-code details, then a review step with the upload progress bar). Step 1 picks or creates the Project and Part inline (`POST /api/projects` / `POST /api/parts` only run if "New" is chosen); step 2 is the same fields as the Upload G-code panel above, pre-filled the same way via `POST /api/gcodes/parse-filename`; the final upload is the exact same `POST /api/gcodes/upload` call, XHR-based for progress, as the per-part panel: this is a second front door onto that endpoint, not a second upload mechanism.
...
- **Admin:** full account management, unchanged from before the uploader role. Lists every user via `GET /api/users`, with an inline "+ Add User" form (`POST /api/users`, role defaults to `uploader`, password optional for an SSO-only account) and a role `<select>` per row (`PUT /api/users/:id`) that updates immediately on change. Removing a user (`DELETE /api/users/:id`) goes through the shared `useConfirm` modal; the server itself refuses to demote or delete the last remaining admin, and refuses to delete the account you're currently signed in as, so this page just surfaces whatever error message comes back rather than duplicating those checks client-side. A row for an unapproved account also shows a "Pending" badge. An `uploader`-role row additionally shows a **Requires print approval** checkbox (`PUT /api/users/:id`, `requires_print_approval`): unrelated to that "Pending" account-approval badge, this instead gates whether *that account's own G-code uploads* stay unapproved until an operator or admin reviews them, see the Projects Page section below and [docs/auth.md](auth.md)'s Print approval section. Hidden for `operator`/`admin` rows, which already have full print authority.
