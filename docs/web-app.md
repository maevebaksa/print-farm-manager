...
- Printers grouped by model: MK4S → Core One → Core 1L → XL → Other (in that order). Each model gets its own self-contained chip, a bordered rectangle (`background: #111827`, `1px solid #1e2433`, matching the Dashboard's `FleetStatusGrid` chips) rather than a full-width row per model: a fleet with many single- or two-printer models packs several narrow chips per line instead of showing mostly-empty full-width rows. A chip's width is a CSS `max(220px + padding, min(100%, N * 230px - 10px + padding, CHIP_MAX_VW vw))`, not a fixed pixel value: exactly as wide as that model's card count needs, capped at `CHIP_MAX_VW` (42) percent of the viewport so a large model wraps internally onto more rows within its own chip instead of growing arbitrarily wide, and never wider than the container so it still shrinks correctly on a narrow viewport. The `padding` term (`CHIP_PADDING_X`, 28px) accounts for the chip's own left/right padding, without which a single-card chip's `minmax(220px, 1fr)` grid column would overflow the chip's border. Because the cap is a live `vw` expression rather than a number computed once in JS, it scales with the window automatically (more columns fit per chip on a wide monitor, fewer on a laptop) with no resize listener needed.
